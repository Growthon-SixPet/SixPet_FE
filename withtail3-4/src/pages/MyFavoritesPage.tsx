...
type SidebarKey = "PROFILE" | "RESERVATIONS" | "REVIEWS" | "FAVORITES";
...
export default function MyFavoritesPage() {
  ...
  const [active, setActive] = useState<SidebarKey>("FAVORITES");
  ...
  const goSidebar = (key: SidebarKey) => {
    setActive(key);
    if (key === "PROFILE") navigate("/mypage/profile");
    if (key === "RESERVATIONS") navigate("/mypage/reservations");
    if (key === "REVIEWS") navigate("/mypage/reviews");
    if (key === "FAVORITES") navigate("/mypage/favorites");
  };
  ...
  const onToggleBookmark = async (place: FavoritePlace) => {
    // TODO: 백엔드 API 연결
    // if (place.type === "HOSPITAL") await api.delete(`/favorites/hospitals/${place.id}`);
    // if (place.type === "FUNERAL") await api.delete(`/favorites/funerals/${place.id}`);
  ...
  const renderSidebarItem = (key: SidebarKey, label: string) => {
    const isActive = active === key;
    return (
      <button
        type="button"
        className={[styles.sidebarItem, isActive ? styles.sidebarItemActive : ""].join(" ")}
        onClick={() => goSidebar(key)}
      >
        {label}
      </button>
    );
    ...
    <div className={styles.pageRoot}>
      ...
      <main className={styles.main}>
        <aside className={styles.sidebar}>
          ...
          {renderSidebarItem("PROFILE", "회원 정보")}
          {renderSidebarItem("RESERVATIONS", "예약 내역 관리")}
          {renderSidebarItem("REVIEWS", "후기 내역 관리")}
          {renderSidebarItem("FAVORITES", "나의 즐겨찾기")}

          <button type="button" className={styles.withdrawItem} onClick={onWithdraw}>
            회원 탈퇴
          </button>
        </aside>
                  ...
                  <div className={styles.cardBody}>
                    ...
                    <div className={styles.infoRow}>
                      ...
                      <div className={styles.infoText}>
                        <div>{p.address}</div>
                        {p.detailAddress ? <div>{p.detailAddress}</div> : null}
                      </div>
                    </div>
            ...
            {Array.from({ length: totalPages }).map((_, idx) => {
              ...
              return (
                <button
                  key={p}
                  type="button"
                  className={[styles.pageNum, isActive ? styles.pageNumActive : ""].join(" ")}
                  onClick={() => setPage(p)}
                >
                  {p}
                </button>
...
function BookmarkIcon({ color = "#FF0000" }: { color?: string }) {
  return (
...
function StarIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
      <path
        d="M12 2L14.9 8.6L22 9.3L16.7 13.8L18.3 21L12 17.3L5.7 21L7.3 13.8L2 9.3L9.1 8.6L12 2Z"
        fill="#F5B301"
      />
    </svg>
  );
